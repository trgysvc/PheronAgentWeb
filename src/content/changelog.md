...
### Added
- **Lark Suite integration (`lark_tool`, UBID 112)** — bridges Lark's official OpenAPI MCP server (`@larksuiteoapi/lark-mcp`) as a bring-your-own-app connection, the same pattern already used for GitHub/Stripe/Zapier: the user creates their own custom app at open.larksuite.com, adds the Bot feature and the needed permission scopes, and pastes the App ID/Secret into Settings > Connections. Tenant_access_token mode — no OAuth flow needed. Live-verified (2026-08-07, real app credentials) exposing **27 tools** across messaging (`im_v1_*`), calendar (`calendar_v4_*`), Base/Bitable (`bitable_v1_*`), docs (`docx_*`), Drive, tasks (`task_v2_*`), and Wiki — the server's default preset alone only exposes 19 tools and omits Calendar entirely, so five presets (`im` + `base` + `doc` + `task` + `calendar`) are combined explicitly. Does not include Mail or any advertising/campaign capability — Lark's API has neither. A step-by-step setup guide is published on the website (Resources > Docs); the MCP Hub's Lark card links to it directly.
- **LemonSqueezy integration (`lemonsqueezy_tool`, UBID 113)** — a generic bridge to LemonSqueezy's official REST API (orders, customers, subscriptions, discounts, license keys, products). No official MCP server exists for LemonSqueezy (every result found is a third-party project), so this talks to the documented REST API directly with the user's own API key, the same pattern as the existing Higgsfield integration. Live-verified with a real account.
- **Kit (formerly ConvertKit) integration (`kit_tool`, UBID 114)** — a generic bridge to Kit's official V4 REST API (subscribers, broadcasts, sequences, tags, forms) using a personal API key from Settings > Developer. Kit does ship an official hosted MCP server, but it requires a browser OAuth flow with dynamic client registration this app doesn't support yet; the simpler documented API-key auth was used instead. Live-verified with a real account.
- **"Disconnect" button added to every MCP Hub service card** (Higgsfield, Perplexity, Stripe, GitHub, Zapier, Notion, Lark, Kit, LemonSqueezy, Unreal Engine, plus Git/Memory/Playwright's live connection) — previously only "Save" existed, with no way to clear a saved credential and tear down a live connection from the UI short of manually deleting it from Keychain.
...
### Fixed
- Corrected the Lark tool's action-name examples from the dotted form Lark's own docs show (`im.v1.message.create`) to the actual snake_case names the server returns (`im_v1_message_create`) — confirmed live via `tools/list`; the dotted form would never have matched a real tool call.
