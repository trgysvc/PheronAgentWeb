...
- **The agent could occasionally refuse a safe, explicitly-requested file/shell operation on its own** instead of attempting it and reporting what actually happened. This showed up as unnecessary clarifying questions even right after the agent had already learned the answer, and as declining a delete/move request without ever checking whether it was actually allowed. The agent now always attempts the operation and relays the tool's real, authoritative outcome.
- **A permanent, non-recoverable restriction (e.g. a sandbox-denied path) could get treated the same as a temporary glitch**, triggering an escalation after just two attempts — which, combined with a separate bug (an internal "was this tool already tried?" record not updating on failure), could loop the agent through the same failed attempt repeatedly. Both fixed; genuinely permanent restrictions are now reported honestly on the first attempt.
- **A generic auto-suggested fix for a failed shell command could actively make things worse for paths starting with `~`** — recommending single-quotes around a path, which disables the shell's home-directory expansion, silently redirecting the operation to an unrelated location while the agent still reported success.
- **A calculation could be silently skipped in a multi-step task** — e.g. after computing a value from a command's output, a follow-up "multiply this by 10" step could get done mentally instead of through the calculator tool. The calculator is now always used for every arithmetic step.
- **The browser-automation tool (Playwright-based) could be called with a wrong action name**, and a successful multi-step browser session could get cut off one step before it could report its result back, due to a turn-budget accounting gap. Both fixed.
- **A tool that failed three times in a row was disabled for the rest of the app session with no way to recover** — even from a one-time, transient failure. It's now automatically given a fresh chance after a short cooldown instead of staying disabled indefinitely.
- **Stripe integration could fabricate a login/OAuth requirement that doesn't exist**, and separately stopped working correctly after Stripe changed its own server's available actions upstream.
...
- **A markdown-list request could come back as plain prose despite the agent finding correct information** — now force-corrected into an actual list.
- **AppleScript-based tools (Calendar and others) could report an unhelpful generic error ("error 0")** instead of the real underlying reason (e.g. a genuine permissions problem). The real reason is now preserved and shown.
- **Adding a Calendar event could silently save it with a completely wrong date** (e.g. a "tomorrow" request landing thousands of years in the future). The date was being handed to Calendar as a raw text string; it's now built from explicit year/month/day/time values instead, and the confirmation message now reports the date actually saved.
- **A file/folder operation could be wrongly rejected as "outside the allowed locations" when the target was the allowed location itself** — the check required something to follow it. A similarly-shaped check could also be tricked by two folders that merely share a name prefix. Both fixed.
- **Moving or copying a file into an existing folder could fail with a confusing "an item with the same name already exists" error** instead of just moving it in. Fixed to match familiar Finder/terminal behavior.
- **A report-writing task could time out and get marked as failed even though the agent was still making genuine progress** — the per-step time budget was tuned for a faster response speed than realistic system load. That budget has been doubled.
- **A "files changed in the last N minutes/hours" request could send the agent reaching for the wrong tool** and stall instead of completing. Clarified which tool actually provides file timestamps.
- **The same request could also waste its remaining steps double-checking an already-correct "nothing found" result** instead of trusting it and answering. The agent now treats an empty result from a timestamp/search command as a complete, trustworthy answer on the first try.
- **A summary of file or command output could occasionally invent specific-sounding details** that were never actually retrieved. Extended the existing research-citation safeguards to also catch a made-up tool/data source in any answer.
- **After the agent corrected a mistake it had just made in a file, it could get blocked from reading the file back to confirm the fix worked** — an internal safeguard treated that follow-up read as a pointless repeat. A read-only check like this is always safe to repeat.
- **A clarifying question from the agent could occasionally vanish entirely, ending the task with a silent blank reply** — caused by a small formatting slip in how the question was generated. Now detected and corrected automatically.
- **When a connected service (e.g. Perplexity) wasn't set up, the agent could repeatedly try substituting other tools or even invent a plausible-sounding fake result** instead of just saying the connection was missing.
...
- **The app could refuse an ordinary request with a "system overloaded" message on a perfectly healthy Mac** — the check was reading macOS's normal, expected RAM usage pattern as if it were a real crisis. Switched to reading the same genuine pressure signal macOS itself uses.
